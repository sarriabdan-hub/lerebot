...
class RealSenseCamera(Camera):
    ...
    def connect(self, warmup: bool = True) -> None:
        ...
        # This device / USB-2 combo stalls if the first frame is polled immediately after
        # start(): try_wait_for_frames() at t~=0 wedges the pipeline for the full timeout.
        # Mirror the proven bare-pipeline pattern (start -> settle -> read) so the read
        # thread's first read lands after the camera has begun streaming.
        time.sleep(1.0)
