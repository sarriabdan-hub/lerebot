...
class EagleBackbone(nn.Module):
    def __init__(
        ...
        # flash-attn isn't installed (no wheel for torch2.11/cu13); force PyTorch SDPA
        # so the Eagle backbone doesn't abort at load. Applies to nested vision/text cfgs too.
        _attn = "flash_attention_2" if use_flash_attention else "sdpa"
        for _cfg in (config, getattr(config, "vision_config", None), getattr(config, "text_config", None)):
            if _cfg is not None:
                _cfg._attn_implementation = _attn
                _cfg._attn_implementation_autoset = True
        self.eagle_model = AutoModel.from_config(config, trust_remote_code=True, attn_implementation=_attn)
