...
class Eagle25VLForConditionalGeneration(Eagle25VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: Eagle25VLConfig, vision_model=None, language_model=None):
        # flash-attn isn't installed on this box (no wheel for torch2.11/cu13); fall back
        # to PyTorch SDPA everywhere so PreTrainedModel.__init__'s attn check doesn't abort.
        import importlib.util as _ilu

        if _ilu.find_spec("flash_attn") is None:
            for _c in (config, getattr(config, "vision_config", None), getattr(config, "text_config", None)):
                if _c is not None:
        ...
        if vision_model is not None:
            ...
            if config.vision_config.model_type == "siglip_vision_model":
                ...
                _fa = _ilu.find_spec("flash_attn") is not None
                config.vision_config._attn_implementation = "flash_attention_2" if _fa else "sdpa"
